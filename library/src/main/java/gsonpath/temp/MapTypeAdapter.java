 ...
 * is registered.
 */
public final class MapTypeAdapter<K, V> extends GsonPathTypeAdapter<Map<K, V>> {
    ...
    @Override
    public Map<K, V> readImpl(JsonReader in, GsonErrors gsonErrors) throws IOException {
        ...
        if (peek == JsonToken.BEGIN_ARRAY) {
            ...
            while (in.hasNext()) {
                ...
                K key;
                if (keyTypeAdapter instanceof GsonPathTypeAdapter) {
                    key = ((GsonPathTypeAdapter<K>) keyTypeAdapter).read(in, gsonErrors);
                } else {
                    key = keyTypeAdapter.read(in);
                }

                V value;
                if (valueTypeAdapter instanceof GsonPathTypeAdapter) {
                    value = ((GsonPathTypeAdapter<V>) valueTypeAdapter).read(in, gsonErrors);
                } else {
    ...
    @Override
    public void writeImpl(JsonWriter out, Map<K, V> map) throws IOException {
        if (map == null) {
            out.nullValue();
            return;
        }
